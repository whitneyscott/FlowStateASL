...
After editing the JSON, re-import or paste into the Canvas Developer Key and install the tool on the account (see Setup & Testing below).

---

## 2. Setup & Testing (how the student opens the tool)

**How homework_submission works:** The tool is NOT attached to the assignment like an External Tool. Instead, when the Developer Key (with `homework_submission`) is installed on the Canvas account, Canvas shows a submission tab for the tool on any assignment that has **Online** → **File Upload** enabled. The student submits via that tab; Canvas launches the tool in an iframe and the tool returns the file.

**Prerequisites (Admin):**

1. Install the Developer Key on the Canvas account: Admin → Developer Keys → create/paste the JSON (from `LTI_1.3_Developer_Key_Canvas_DEV_PROMPTER.json` or prod).
2. Enable the key and install it in the account/course (as configured in your Canvas instance).

**Teacher setup:**

1. Create or edit an assignment.
2. Set **Submission type** to **Online**.
3. Check **File Upload** (optionally also Text Entry).
4. Save.

**Student flow:**

1. Open the assignment and click **Submit Assignment**.
2. In the submission area, Canvas shows tabs such as "File", "Canvas Files", and **"ASL Express – Record video"** (the `text` from the homework_submission placement).
3. Student clicks the **ASL Express – Record video** tab → Canvas launches the LTI tool in an iframe with `LtiDeepLinkingRequest`.
4. Student sees prompts, records video, submits → tool posts the Deep Linking response → Canvas attaches the video to the submission.

No External Tool configuration is needed on the assignment. The tool appears automatically for any File Upload assignment when the Developer Key is installed.

---

## 3. Deep Linking launch flow (end to end)

- **Same launch URL:** Canvas uses the single LTI 1.3 launch URL; the **message_type** in the JWT distinguishes Resource Link vs Deep Linking.
- **Iframe stays open:** Student sees prompts and records inside the tool; after the tool posts the Deep Linking response, Canvas closes/updates the iframe and attaches the file.
- Tool flow: receive Deep Linking request and `deep_link_return_url`; host video via one-time token; post LtiDeepLinkingResponse JWT via form POST; Canvas GETs file and attaches to submission; frontend branches on `messageType === 'LtiDeepLinkingRequest'` and uses submit-deep-link path.

---

## 9. Risks and Canvas-specific quirks
...
6. E2E: create a File Upload assignment (as teacher), submit as student via the "ASL Express – Record video" tab, confirm file appears in submission.
7. Shadow assignment and grading (section 10): config storage, submission detection, teacher UI, grade submission to shadow.

---

## 11. Shadow assignment and grading
...
### 11.3 Detecting submissions and showing count
...
### 11.4 Teacher opens videos and submits grades to shadow
