...
export function appendSignedQueryToCourseImageViewPath(
  ...
  const payload = `v1|${fid}|${cid}|${expSec}`;
  const sig = hmacHex(secret, payload);
  const q = new URLSearchParams({ c: cid, e: String(expSec), sig });
...
export function applySignedCourseImageViewsToConfig(
  ...
  if (Array.isArray(next.prompts) && next.prompts.length > 0) {
    next.prompts = next.prompts.map((p) => rewriteHtmlWithSignedCourseImageViews(String(p ?? ''), cid, config) ?? '');
  }
  if (typeof next.instructions === 'string' && next.instructions.trim()) {
    next.instructions = rewriteHtmlWithSignedCourseImageViews(next.instructions, cid, config) ?? next.instructions;
  }
  return next;
}
