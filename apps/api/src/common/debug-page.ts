      ...
      const val =
        v === null || v === undefined
          ...
          : typeof v === 'object'
            ...
            : String(v);
      return `<tr><th>${escapeHtml(k)}</th><td><pre>${escapeHtml(val)}</pre></td></tr>`;
    })
    .join('');
  return `<!DOCTYPE html>
<html>
<head><title>ASL Express Debug</title>
<style>body{font-family:sans-serif;margin:2em;background:#f5f5f5} table{border-collapse:collapse;background:white;box-shadow:0 1px 3px rgba(0,0,0,.1)} th,td{border:1px solid #ddd;padding:8px 12px;text-align:left;vertical-align:top} th{width:220px;background:#eee} pre{margin:0;white-space:pre-wrap;word-break:break-all} h1{color:#333} .error{color:#c00}</style>
</head>
<body>
